...
class ImageData {
 public:
  ...
  void CopyImage(const ImageData<T>& src_img);
#if 0
...
template <class T>
void ImageData<T>::CopyImage(const ImageData<T>& src_img) {
  m_file_name = src_img.m_file_name;
  m_width = src_img.m_width;
  m_height = src_img.m_height;
  m_random_seed = src_img.m_random_seed;
  if (m_data != NULL) {
    delete m_data;
    m_data = NULL;
  }
  m_data = new std::vector<T>(*(src_img.m_data));
}
